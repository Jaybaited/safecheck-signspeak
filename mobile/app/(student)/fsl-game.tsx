...
export default function FSLGameScreen() {
  ...
  }, [gameMode, getRandomLetter, endGame]);

  const startScanning = useCallback(() => {
    if (scanRef.current) clearInterval(scanRef.current);
    scanRef.current = setInterval(captureAndCheck, 1200);
  }, [captureAndCheck]);
      ...
      <SafeAreaView style={[s.container, { backgroundColor: C.background }]}>
        ...
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[s.card, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={[s.modeHeader, { backgroundColor: "#FFF7ED", borderRadius: 16, padding: 14 }]}>
              <View style={[s.iconBox, { backgroundColor: "#F97316" }]}>
                <Zap size={24} color="#fff" />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[s.cardTitle, { color: C.text }]}>Speed Challenge</Text>
                <Text style={[s.cardDesc, { color: C.muted }]}>
                  Sign as many letters as you can in{" "}
                  <Text style={{ color: "#F97316", fontWeight: "700" }}>60 seconds</Text>.
                </Text>
              </View>
            </View>
            <View style={[s.metaRow, { marginTop: 4 }]}>
              <View style={[s.metaPill, { backgroundColor: "#FEF3C7" }]}>
                <Clock size={12} color="#B45309" />
                <Text style={[s.metaText, { color: "#B45309" }]}>60 seconds</Text>
              </View>
              <View style={[s.metaPill, { backgroundColor: "#FEF3C7" }]}>
                <Star size={12} color="#B45309" />
                <Text style={[s.metaText, { color: "#B45309" }]}>Score based</Text>
              </View>
            </View>
            <TouchableOpacity
          ...
          {/* Letter Streak */}
          <View style={[s.card, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={[s.modeHeader, { backgroundColor: "#F5F3FF", borderRadius: 16, padding: 14 }]}>
              <View style={[s.iconBox, { backgroundColor: "#8B1A1A" }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[s.cardDesc, { color: C.muted }]}>
                  Sign{" "}
                  <Text style={{ color: "#8B1A1A", fontWeight: "700" }}>{STREAK_TARGET} letters</Text>
                  {" "}in a row. No timer — just accuracy.
                </Text>
              </View>
            </View>
            <View style={[s.metaRow, { marginTop: 4 }]}>
              <View style={[s.metaPill, { backgroundColor: "#FEE2E2" }]}>
                <Target size={12} color="#8B1A1A" />
                <Text style={[s.metaText, { color: "#8B1A1A" }]}>{STREAK_TARGET} letters</Text>
              </View>
              <View style={[s.metaPill, { backgroundColor: "#FEE2E2" }]}>
                <Star size={12} color="#8B1A1A" />
                <Text style={[s.metaText, { color: "#8B1A1A" }]}>Accuracy based</Text>
              </View>
            </View>
            <TouchableOpacity
              style={[s.playBtn, { backgroundColor: "#8B1A1A" }]}
              onPress={startStreakMode}
            >
              <Play size={16} color="#fff" />
              <Text style={s.playBtnText}>Play Now</Text>
            </TouchableOpacity>
          ...
          {/* How to Play */}
          <View style={[s.card, { backgroundColor: C.card, borderColor: C.border }]}>
            <Text style={[s.cardTitle, { color: C.text, marginBottom: 4 }]}>How to Play</Text>
            {[
      ...
      <SafeAreaView style={[s.container, { backgroundColor: C.background }]}>
        ...
        <View style={s.hud}>
          <View style={[s.hudBox, { backgroundColor: C.card, borderColor: C.border }]}>
            ...
            <Text style={[s.hudValue, { color: C.primary }]}>{score.toLocaleString()}</Text>
          </View>

          {gameMode === "speed" && (
            <View style={[s.hudBox, { backgroundColor: C.card, borderColor: timeLeft <= 10 ? "#EF4444" : C.border }]}>
              <Text style={[s.hudLabel, { color: C.muted }]}>Time</Text>
              <Text style={[s.hudValue, { color: timeLeft <= 10 ? "#EF4444" : "#F97316" }]}>{timeLeft}s</Text>
              <View style={[s.timerBarBg, { backgroundColor: C.border }]}>
                <View style={[s.timerBar, {
                  width: `${(timeLeft / SPEED_DURATION) * 100}%`,
          ...
          {gameMode === "streak" && (
            <View style={[s.hudBox, { backgroundColor: C.card, borderColor: C.border }]}>
              ...
              <Text style={[s.hudValue, { color: C.primary }]}>
                {lettersCompleted.length}
                <Text style={{ fontSize: 14, color: C.muted }}>/{STREAK_TARGET}</Text>
              </Text>
              <View style={[s.timerBarBg, { backgroundColor: C.border }]}>
                <View style={[s.timerBar, {
                  width: `${(lettersCompleted.length / STREAK_TARGET) * 100}%`,
          ...
          <Animated.View style={[
            s.targetBox,
            { backgroundColor: flash === "correct" ? "#D1FAE5" : C.primary },
            { opacity: flashAnim.interpolate({ inputRange: [0, 1], outputRange: [1, 0.6] }) },
          ]}>
            <Text style={[s.targetLabel, { color: flash === "correct" ? "#059669" : "#FECACA" }]}>
              {flash === "correct" ? "✅ Correct!" : "Sign this"}
            </Text>
            <Text style={[s.targetLetter, { color: flash === "correct" ? "#059669" : "#fff" }]}>
              {currentLetter}
            </Text>
          </Animated.View>
        </View>
        ...
        <View style={[s.cameraWrap, { borderColor: C.border }]}>
          <CameraView ref={cameraRef} style={s.camera} facing="front">
            <View style={[s.badge, { top: 12, right: 12, backgroundColor: handDetected ? "#059669" : "#374151" }]}>
              <Text style={s.badgeText}>{handDetected ? "● Hand Detected" : "○ No Hand"}</Text>
            </View>
            <View style={[s.badge, { top: 12, left: 12, backgroundColor: "rgba(0,0,0,0.6)" }]}>
              <Text style={s.badgeText}>Sign: {currentLetter}</Text>
            </View>
          </CameraView>

          <View style={[s.liveCard, { backgroundColor: C.card }]}>
        ...
        {lettersCompleted.length > 0 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.chips}>
            {lettersCompleted.slice(-12).map((l, i) => (
              <View key={i} style={[s.chip, { backgroundColor: C.border }]}>
    ...
    return (
      <SafeAreaView style={[s.container, { backgroundColor: C.background }]}>
        ...
        <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[s.card, { backgroundColor: C.card, borderColor: C.border, alignItems: "center" }]}>
            <View style={[s.trophyWrap, { backgroundColor: isNewHigh ? "#FEF9C3" : C.inputBg }]}>
              <Trophy size={48} color={isNewHigh ? "#EAB308" : C.muted} />
            </View>
            {isNewHigh && (
            ...
            <View style={s.statsRow}>
              <View style={[s.statBox, { backgroundColor: C.inputBg }]}>
                <Text style={[s.statVal, { color: C.primary }]}>{result.score.toLocaleString()}</Text>
                <Text style={[s.statLabel, { color: C.muted }]}>Score</Text>
              </View>
              <View style={[s.statBox, { backgroundColor: C.inputBg }]}>
                <Text style={[s.statVal, { color: "#10B981" }]}>{result.lettersCompleted.length}</Text>
                <Text style={[s.statLabel, { color: C.muted }]}>Letters</Text>
              </View>
              <View style={[s.statBox, { backgroundColor: C.inputBg }]}>
                <Text style={[s.statVal, { color: "#F97316" }]}>
            ...
            <View style={s.resultBtns}>
              <TouchableOpacity
                style={[s.playBtn, { flex: 1, backgroundColor: C.primary }]}
              ...
              <TouchableOpacity
                style={[s.playBtn, { flex: 1, backgroundColor: C.card, borderWidth: 1, borderColor: C.border }]}
            ...
            <View style={[s.card, { backgroundColor: C.card, borderColor: C.border }]}>
              ...
              <View style={s.completedGrid}>
                {result.lettersCompleted.map((l, i) => (
                  <View key={i} style={[s.completedChip, { backgroundColor: C.border }]}>
...
const s = StyleSheet.create({
  ...
  targetBox:          { flex: 1, borderRadius: 16, padding: 10, alignItems: "center", justifyContent: "center" },
  ...
  completedGrid:      { flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 4 },
