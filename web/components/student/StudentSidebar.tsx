...
const ACTIVE_CLS   = 'bg-[#7B1113]/10 dark:bg-[#7B1113]/20 text-[#7B1113] dark:text-[#E8C96A]';
const INACTIVE_CLS = 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white';
  ...
  return (
    <>
      <aside className="fixed left-0 top-0 h-screen w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col">
        ...
        {/* Navigation */}
        <nav className="flex-1 overflow-y-auto p-4 space-y-1" aria-label="Student navigation">
          {TOP_NAV.map((item) => {
            const isActive = pathname === item.href;
            return (
              <Link key={item.name} href={item.href}>
                <div className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-colors ${isActive ? ACTIVE_CLS : INACTIVE_CLS}`}>
          ...
          <div>
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isFslActive ? ACTIVE_CLS : INACTIVE_CLS}`}
            >
              <FSL_GROUP.icon className="w-5 h-5 shrink-0" />
              <span className={`flex-1 text-left ${isFslActive ? 'font-medium' : ''}`}>
                {FSL_GROUP.label}
              </span>
              <ChevronDown className={`w-4 h-4 shrink-0 transition-transform duration-200 ${fslOpen ? 'rotate-180' : 'rotate-0'}`} />
            </button>
            <div
              id="fsl-submenu"
