...
export default function FSLProgressPage() {
  ...
  const avgCompleted = Math.round(
    SAMPLE_PROGRESS.reduce((acc, s) => acc + s.completed, 0) / SAMPLE_PROGRESS.length
  );
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 text-slate-900 dark:text-white transition-colors duration-200">
      ...
      <main className="ml-64 p-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-gray-400 text-sm">
              Monitor your students&apos; Filipino Sign Language progress
            </p>
          </div>
          <ThemeToggle />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {[
            {
              icon: Users,      label: 'Total Students', color: 'text-slate-900 dark:text-white',
              iconCls: 'text-cyan-600 dark:text-cyan-400',    bg: 'bg-cyan-100 dark:bg-cyan-500/10',
              value: SAMPLE_PROGRESS.length,
            },
            {
              icon: TrendingUp, label: 'Class Avg',      color: 'text-purple-600 dark:text-purple-400',
              iconCls: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-100 dark:bg-purple-500/10',
              value: `${avgPct}%`,
            },
            {
              icon: Award,      label: 'Above 75%',      color: 'text-emerald-600 dark:text-green-400',
              iconCls: 'text-emerald-600 dark:text-green-400', bg: 'bg-emerald-100 dark:bg-green-500/10',
              value: SAMPLE_PROGRESS.filter((s) => (s.completed / s.total) >= 0.75).length,
            },
            {
              icon: BookOpen,   label: 'Needs Attention', color: 'text-orange-600 dark:text-orange-400',
              iconCls: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-100 dark:bg-orange-500/10',
              value: SAMPLE_PROGRESS.filter((s) => (s.completed / s.total) < 0.5).length,
            },
        ...
        <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl shadow-sm dark:shadow-none transition-colors duration-200">
          ...
          <table className="w-full">
            ...
            <tbody>
              {sorted.map((s) => {
                ...
                return (
                  // ✅ Fragment with key — fixes the "unique key" warning
                  <Fragment key={s.id}>
                    ...
                    {/* Expanded letter grid */}
                    {isOpen && (
                      <tr className="bg-slate-50 dark:bg-gray-800/40">
          ...
          {sorted.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-slate-400 dark:text-gray-500 text-sm">No students found</p>
            </div>
          )}
        </div>
      </main>
