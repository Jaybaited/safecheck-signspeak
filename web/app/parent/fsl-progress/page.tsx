    ...
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 text-slate-900 dark:text-white transition-colors duration-200">
      ...
      <main className="ml-64 p-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-1">FSL Progress</h1>
            <p className="text-slate-500 dark:text-gray-400 text-sm">
              {child ? `${child.firstName}'s` : "Child's"} Filipino Sign Language learning progress
            </p>
          </div>
          ...
          {[
            ...
            <div key={label} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl p-6 shadow-sm dark:shadow-none transition-colors duration-200">
              <div className="flex items-center justify-between mb-4">
                ...
                <TrendingUp className="w-5 h-5 text-slate-300 dark:text-gray-600" />
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl p-6 shadow-sm dark:shadow-none transition-colors duration-200">
            ...
            <div className="flex items-end gap-3 h-40">
              {weeklyActivity.map(({ day, count }) => (
                <div key={day} className="flex-1 flex flex-col items-center gap-2">
                  <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">{count > 0 ? count : ''}</span>
                  <div className="w-full flex flex-col justify-end" style={{ height: '120px' }}>
                    <div
                      className="w-full bg-[#7B1113] dark:bg-[#9B2020] rounded-t-md transition-all duration-500 hover:opacity-80"
            ...
            <p className="text-xs text-slate-400 dark:text-gray-500 mt-4 text-center">Number of letters practiced per day</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl p-6 shadow-sm dark:shadow-none transition-colors duration-200">
            ...
            <div className="space-y-3">
              {dynamicAchievements.map(({ title, earned, icon: Icon, xp }) => (
                <div key={title} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                  earned ? 'bg-[#7B1113]/5 dark:bg-[#7B1113]/10 border-[#7B1113]/20 dark:border-[#7B1113]/30'
                         : 'bg-slate-50 dark:bg-gray-800/50 border-slate-200 dark:border-gray-700 opacity-50'
                }`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                    earned ? 'bg-[#7B1113] text-white' : 'bg-slate-200 dark:bg-gray-700 text-slate-400 dark:text-gray-500'
                  }`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${earned ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-gray-400'}`}>{title}</p>
                    <p className={`text-xs ${earned ? 'text-yellow-600 dark:text-yellow-400' : 'text-slate-400 dark:text-gray-500'}`}>+{xp} XP</p>
                  </div>
                  {earned && <CheckCircle className="w-4 h-4 text-[#7B1113] dark:text-[#E8C96A] shrink-0" />}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl p-6 shadow-sm dark:shadow-none transition-colors duration-200">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold">FSL Alphabet Map</h2>
              <p className="text-sm text-slate-500 dark:text-gray-400 mt-1">{completedLetters.size} of {fslLetters.length} letters mastered</p>
            </div>
            <span className="text-2xl font-bold text-[#7B1113] dark:text-[#E8C96A]">{pct}%</span>
          </div>
          <div className="w-full bg-slate-100 dark:bg-gray-800 rounded-full h-2 mb-6">
            <div className="bg-[#7B1113] h-2 rounded-full transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
            ...
            {fslLetters.map((l) => {
              ...
              return (
                <div key={l} className={`aspect-square rounded-xl flex flex-col items-center justify-center text-sm font-bold transition-all ${
                  done ? 'bg-[#7B1113] text-white shadow-md' : 'bg-slate-100 dark:bg-gray-800 text-slate-400 dark:text-gray-500'
                }`}>
                  {l}
