...
// Counts consecutive school days (Mon–Fri) with attendance going back from today
function calcStreak(records: AttendanceRecord[]): number {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    if (presentDates.has(iso)) {
      ...
      if (i === 0) continue; // allow today to still be pending
      break;
    }
  }
  return streak;
...
export default function StudentProgressPage() {
    ...
    try {
      ...
      Promise.all([
        ...
        .then(([stats, records]: [AttendanceStats, AttendanceRecord[]]) => {
          ...
          // XP = 50 per letter + game bonus + quiz bonus
          const xp = letters.size * 50 + Math.floor(hs / 10) + qb * 20;
          setTotalXP(xp);
        })
        .catch(() => {
          // Fallback: show local data only, no attendance
          setWeekDays(weekDates.map(({ label, iso }) => ({
            label, iso,
            fsl:      studentStorage.getFslActivity(parsedUser.id, iso),
            attended: false,
  ...
  const achievements = [
    {
      id: 1, title: 'First Sign',
      desc: 'Completed your first FSL letter',
      icon: Star, xp: 50,
      earned: completedLetters.size >= 1,
    },
    {
      id: 2, title: 'Half the Alphabet',
      desc: 'Mastered 12 or more letters',
      icon: BookOpen, xp: 200,
      earned: completedLetters.size >= 12,
    },
    {
      id: 3, title: 'Full Alphabet',
      desc: 'Mastered all 24 FSL letters',
      icon: Award, xp: 500,
      earned: completedLetters.size >= 24,
    },
    {
      id: 4, title: 'Perfect Week',
      desc: 'Attended all 5 weekdays this week',
      icon: Calendar, xp: 150,
      earned: weekDays.filter(d => ['Mon','Tue','Wed','Thu','Fri'].includes(d.label) && d.attended).length >= 5,
    },
    {
      id: 5, title: 'Week Streak',
      desc: '7-day attendance streak',
      icon: Flame, xp: 100,
      earned: streak >= 7,
    },
    {
      id: 6, title: 'High Scorer',
      desc: 'Score 500+ points in FSL Games',
      icon: Target, xp: 300,
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 text-slate-900 dark:text-white transition-colors duration-200">
      ...
      <main className="ml-64 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          ].map(({ icon: Icon, label, value, sub, color, bg, iconCls }) => (
            <div
              key={label}
