...
export default function FSLQuizPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setUser(parsedUser);
      // ✅ Student-scoped best score
      const saved = studentStorage.get(parsedUser.id, 'fsl_quiz_best');
  ...
  const handleNext = () => {
    if (currentIdx < questions.length - 1) {
      ...
      const total = questions.filter((q, i) =>
        (i === currentIdx ? selected : answers[i]) === q.correct
      ).length;

      if (total > bestScore && user) {
        setBestScore(total);
        // ✅ Student-scoped save
        studentStorage.set(user.id, 'fsl_quiz_best', String(total));
      }
      setQuizState('review');
    }
  };
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl p-8 text-white text-center shadow-lg shadow-purple-500/20">
              ...
              <p className="text-5xl font-bold mb-1">
                {bestScore}<span className="text-2xl text-purple-300">/{QUIZ_LENGTH}</span>
              </p>
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl p-8 shadow-sm dark:shadow-none transition-colors duration-200">
                ...
                {currentQ.options.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 text-lg font-bold transition-all ${style} disabled:cursor-default`}
                    >
                      {option}
                    </button>
                  );
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className={`text-sm font-semibold mb-1 ${
                        ...
                        {selected === currentQ.correct ? 'Correct!' : selected === '__timeout__' ? "Time's up!" : 'Incorrect'}
                      </p>
                      <p className="text-sm text-slate-600 dark:text-gray-400">{currentQ.explanation}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {selected !== null && (
                <button
                  onClick={handleNext}
                  className="w-full mt-4 flex items-center justify-center gap-2 py-3 bg-purple-600 hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600 text-white rounded-lg font-medium transition-colors"
                >
        ...
        {quizState === 'review' && (
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className={`rounded-2xl p-8 text-white text-center shadow-lg ${
              pct >= 80 ? 'bg-gradient-to-r from-emerald-500 to-teal-500'
              ...
              <p className="text-5xl font-bold mb-1">
                {finalScore}<span className="text-2xl text-white/70">/{questions.length}</span>
              </p>
              <p className="text-white/80 text-sm">{pct}% correct</p>
            </div>
            ...
            <div className="space-y-3">
              {questions.map((q, i) => {
                ...
                return (
                  <div
                    key={q.id}
                    className={`bg-white dark:bg-gray-900 border rounded-xl p-5 shadow-sm dark:shadow-none transition-colors duration-200 ${
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-sm">
                          <span className={`font-bold px-2 py-0.5 rounded ${
                            ...
                            Your answer: {userAnswer === '__timeout__' ? 'Timed out' : userAnswer}
                          </span>
                          {!isCorrect && (
                            <span className="font-bold px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">
                              Correct: {q.correct}
                            </span>
                          )}
                        </div>
